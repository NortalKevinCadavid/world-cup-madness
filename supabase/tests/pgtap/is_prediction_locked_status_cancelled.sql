-- Slice 003 / T023 / contracts/prediction-lock.predicate.sql.md § Test surface row 8. Calibrated to now() so the test is location-independent.
--
-- BR-LOCK-004: status='cancelled' locks the match. Synthesized inside
-- the txn.
...
SELECT is(
  ...
  true,
  'status_cancelled: status=cancelled with kickoff 24h out -> predicate TRUE (BR-LOCK-004)'
);
